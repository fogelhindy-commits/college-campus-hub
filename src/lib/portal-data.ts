...
export const INITIAL_STATE: PortalDataStore = {
  users: [
    {
      id: "owner-olivia",
      name: "Olivia Carter",
...
async function loadStoreFromDatabase() {
  try {
    ...
    if (!row) {
      try {
        await bootstrapAppStateRow();
      } catch (bootstrapError) {
        console.error("Failed to bootstrap portal state row", bootstrapError);
      }
    ...
    console.error("Failed to load portal state from database, bootstrapping fallback", error);
    try {
      await bootstrapAppStateRow();
    } catch (bootstrapError) {
      console.error("Failed to bootstrap portal state row", bootstrapError);
...
async function bootstrapAppStateRow() {
  await prisma.$executeRawUnsafe(APP_STATE_SQL);
  await prisma.appState.upsert({
    where: { id: APP_STATE_ID },
    create: {
      id: APP_STATE_ID,
      data: JSON.stringify(cloneStore(INITIAL_STATE)),
    },
    update: {
      data: JSON.stringify(cloneStore(INITIAL_STATE)),
    },
...
export async function seedPortalState() {
  await persistStoreToDatabase(cloneStore(INITIAL_STATE));
}

let storeCache = IS_BUILD_TIME
  ? cloneStore(INITIAL_STATE)
  : await loadStoreFromDatabase().catch((error) => {
      console.error("Failed to load college app state, using in-memory fallback", error);
      return cloneStore(INITIAL_STATE);
    });
