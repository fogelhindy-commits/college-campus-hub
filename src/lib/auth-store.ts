  ...
  try {
    ...
    if (!row) {
      try {
        ...
        console.error("Failed to bootstrap auth state row", bootstrapError);
      }
      return cloneState(INITIAL_AUTH_STATE);
    }
    ...
    console.error("Failed to load auth state, using in-memory fallback", error);
    try {
      await bootstrapAuthState();
    } catch (bootstrapError) {
      console.error("Failed to bootstrap auth state row", bootstrapError);
...
async function bootstrapAuthState() {
  ...
  await prisma.appState.upsert({
    ...
    create: {
      id: AUTH_STATE_ID,
      data: JSON.stringify(cloneState(INITIAL_AUTH_STATE)),
    },
    update: {
      data: JSON.stringify(cloneState(INITIAL_AUTH_STATE)),
    },
